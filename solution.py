...
def _summation_of_intersections_of_n_size(mountains: list, intersection_size: int):
    intersection_area = 0
    for index, mountain in enumerate(mountains): 
        temp_mountains = []
        temp_mountains.append(mountain)
        for _, next_mountain in enumerate(mountains[index+1:index+intersection_size]):
            if(mountain['right'] >= next_mountain['left']):
                temp_mountains.append(next_mountain)
        if len(temp_mountains) == intersection_size:
